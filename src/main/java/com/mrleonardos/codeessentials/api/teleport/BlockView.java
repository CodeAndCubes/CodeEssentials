...
public interface BlockView {
    ...
    /**
     * Загружен ли чанк с этой колонкой прямо сейчас.
     *
     * <p>
     * Ответ обязан быть дешёвым: поиск спрашивает про каждую колонку кольца, и чтение region-файла
     * на каждый вопрос встало бы сервером на целый тик. {@link #sample} спрашивают только про
     * загруженные чанки: незагруженный чанк отдал бы воздух и увёл бы игрока под мир.
     */
    boolean chunkLoaded(int blockX, int blockZ);

    /**
     * Поднять чанк с этой колонкой и сказать, доступна ли она теперь.
     *
     * <p>
     * Единственное место, где поиск платит за диск, поэтому спрашивают только про целевую колонку и
     * её соседей. Чанка нет на диске и генерация выключена: ответ ложь, работа получает
     * {@code CHUNK_MISSING}.
     */
    default boolean bringUpChunk(int blockX, int blockZ) {
     ...
     *
     * <p>
     * Спрашивается до подъёма чанка: в 1.7.10 биом считается по шуму генератора и чтения с диска не
     * требует, поэтому отсев по чёрному списку биомов ничего не стоит.
     *
     * @return имя биома или пустая строка, если мир его не называет. Пустой ответ означает «не
     *         знаю», и чёрный список биомов тогда никого не отсеивает
     */
    default String biome(int blockX, int blockZ) {
        return "";
    }
